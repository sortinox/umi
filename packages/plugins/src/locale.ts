...
export default (api: IApi) => {
  ...
  api.onGenerateFiles(async () => {
    const localeTpl = readFileSync(join(LOCALE_TEMPLATES_DIR, "locale.tpl"), "utf-8")
    // moment2dayjs
    const resolveKey = api.config.moment2dayjs ? "dayjs" : "moment"
    const momentPkgPath = winPath(dirname(require.resolve(`${resolveKey}/package.json`)))
    const EventEmitterPkg = winPath(dirname(require.resolve("event-emitter/package")))
    ...
    const localeExportsTpl = readFileSync(join(LOCALE_TEMPLATES_DIR, "localeExports.tpl"), "utf-8")
    ...
    // runtime.tsx
    const runtimeTpl = readFileSync(join(LOCALE_TEMPLATES_DIR, "runtime.tpl"), "utf-8")
    api.writeTmpFile({
      path: "runtime.tsx",
    ...
    // SelectLang.tsx
    const selectLang = readFileSync(join(LOCALE_TEMPLATES_DIR, "SelectLang.tpl"), "utf-8")
