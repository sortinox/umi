...
import { Loader, transformSync as transformSync2, type TransformResult } from "@umijs/bundler-utils/compiled/esbuild"
...
export class ModelUtils {
  ...
  static topologicalSort = (models: Model[]) => {
    ...
    if (leftNodes.length > 0) {
      throw new Error(`Circle dependency detected in models: ${leftNodes.map((m) => chalk.red(m.namespace)).join(", ")}`)
    }
