...
export default async ({ cwd, args, defaultData = DEFAULT_DATA }: IGeneratorOpts) => {
  ...
  const isPnpmHighestResolution = isPnpm8 && semver.gte(pnpmVersion!, pnpmHighestResolutionMinVersion)
  if (!useDefaultData && args.install !== false) {
    ...
    if (isPnpm8) {
      logger.warn(chalk.yellow(`You current using pnpm v8, it will install minimal version of dependencies`))
      logger.warn(chalk.green(`Recommended that you run ${chalk.bold.cyan("pnpm up -L")} to install latest version of dependencies`))
    }
  }
}
