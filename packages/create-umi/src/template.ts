...
export const unpackTemplate = async (opts: IUnpackTemplateOpts) => {
  ...
  logger.info(`Init a new project with template ${chalk.blue(template)} from npm ...`)
  ...
  // not found
  throw new Error(`Template ${nameList.map((i) => chalk.yellow(i)).join(", ")} not found from ${registry}`)
}
